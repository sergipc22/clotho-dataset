...
def feature_extraction(audio_data: np.ndarray, sr: int, nb_fft: int,
    ...
    gammatone_spec = gtgram.gtgram(
        wave=y, fs=sr, window_time=nb_fft/sr, hop_time=hop_size/sr,
        channels=nb_mels, f_min=f_min).T

    return np.flipud(20 * np.log10(gammatone_spec + np.finfo(float).eps))
